...
func play_route(upper: bool) -> void:
	...
	for tick in range(10000):
		...
		if stage == 0 and player.position.x > 2000:
			stage = 1
			travel_direction = -1
			jumps = [1572,1419,1344,1210,1052,909,704,608,528] if upper else [1572,1065,925,619,555,503]
			jump_index = 0
			print("RETURN VIA ","UPPER" if upper else "LOWER")
		if stage == 1 and player.position.x < 430:
			stage = 2
			travel_direction = 1
			jumps = [852,1004,1267,1331,1390,1515] if upper else [470,548,645,820,999,1160,1280,1365,1515]
			jump_index = 0
			print("COLLECT OTHER BRANCH")
		Input.action_press("move_right" if travel_direction > 0 else "move_left")
		if jump_frames > 0:
			jump_frames -= 1
		else:
			Input.action_release("jump")
		if jump_index < jumps.size() and (player.position.x >= jumps[jump_index] if travel_direction > 0 else player.position.x <= jumps[jump_index]) and player.is_on_floor():
		...
		if stage == 2 and level.gate.player_near() and not level.gate.opened:
		...
		if stuck>240:
			...
			break
	release_inputs()
	var ok: bool = level.finished and not player.dead and level.gold == 6
	...
	if DisplayServer.get_name()!="headless":
		...
		root.get_texture().get_image().save_png("res://work/backtrack-"+("upper" if upper else "lower")+".png")
	level.queue_free()
	await process_frame
func run() -> void:
