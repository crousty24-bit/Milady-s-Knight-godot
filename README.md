...
**Le dépôt contient actuellement une scène de test jouable. RUN-001 stabilise le moteur, l’import et les tests ; sa validation humaine reste à terminer.** La cible finale est une démo **0.9.0 beta**, pas une release 1.0.0. Le badge `0.1.0` désigne le prochain jalon ; les tests couvrent les règles actuelles du prototype.
...
- Test graphique de notification de fermeture réussi. Le contrôle manuel **F5 puis fermeture de l’éditeur** reste à confirmer ; les preuves et limites sont dans [runs-journal.md](runs-journal.md). Le binaire Linux 4.7.2 n’a pas été testé sur cette machine.
