...
**Contrôle reporté avant distribution :** obtenir pour chacun des cinq fichiers retenus le fichier/pack d'origine exact, auteur, URL officielle ou archive d'achat, version, licence, attribution et chaîne de transformation. Si ces éléments manquent lors de cette recette, remplacer le média par une source documentée et tester son intégration. Les étiquettes `Free` et `DL ✅` des listes de candidats ne suffisent pas à valider un droit d'usage.

L'humain indique que chacun des assets employés possède une licence accessible depuis son lien source, avec des formulations variables mais les mêmes principes : usage personnel/commercial dans un jeu et modifications permis, redistribution des fichiers d'assets modifiés ou non interdite, crédit facultatif. Il n'a pas de justificatifs reliés aux fichiers actuels et demande de différer cette recherche. Les liens sources des cinq médias audio ci-dessus ne sont donc pas identifiés ; vérifier leurs conditions respectives lors de leur sélection finale, sans attribuer dès maintenant une licence précise à ces fichiers.
