...
Audit du **21 septembre 2026**, complété par les vérifications et runs clôturées jusqu'au **25 septembre 2026**. Les preuves d’exécution sont dans [runs-journal.md](runs-journal.md) ; l’audit initial et la planification restent dans [runs-workflow.md](runs-workflow.md).

### Version-cible actuelle

**0.1.0 validée comme cible : socle de production vérifié.** La roadmap va jusqu’à **0.9.0 beta**, démo finale. RUN-001 à RUN-006 sont terminées et leurs PR fusionnées dans `develop` ; RUN-007 reste en backlog. Les catalogues 11–12 sont locaux et le rattachement des licences aux fichiers hérités est différé à la recette des assets retenus. Le dépôt reste une scène de test issue du vertical slice, pas encore le niveau 1 conforme aux nouvelles spécifications.
...
- **Joueur :** marche accélérée, saut variable, double saut, coyote/buffer, wall slide/wall jump, épée au sol/en l’air avec déduplication et occlusion ; santé fractionnaire, profils de réactions aux dégâts, mort et reprise avec E sur l'overlay. Les flèches remplacent Q/D/Z/S ; F répète les frappes tant qu'il est maintenu. Les durées actuelles de 0,18 s de hit-stun et 0,16 s de recul sont provisoires.
...
- **Vérification actuelle :** sous Windows **4.7.2**, le dernier essai de RUN-006 a réussi l'import propre, l'isolation `user://`, **12 suites et 243 contrôles de jeu** ; trois captures HUD ont été inspectées. Le correctif et le ressenti ont été validés par l'humain le 25 septembre 2026. Les erreurs `p_position > length` observées lors de l'audit initial étaient résolues par le remplissage RIFF des trois WAV ; les essais initiaux sur copies WSL/UNC et disque local sont détaillés dans le journal.
